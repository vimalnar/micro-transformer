...
### Example rendered observations

These are authentic 128×128 PNGs generated by the Python canonical renderer from
the agent's 9×9 observation, using the same `physics-lab` map and seed. The
simulation was advanced with `wait` actions to the indicated ticks. They show
the observation available to the agent—not the full 16×16 world or a screenshot
of the browser inspector. Lighting changes the visible field across day phases.

| Day · tick 0 | Dusk · tick 60 | Night · tick 70 |
| --- | --- | --- |
| ![Canonical agent observation in daylight at tick 0](../concepts/micro-world-observation-day.png) | ![Canonical agent observation at dusk at tick 60](../concepts/micro-world-observation-dusk.png) | ![Canonical agent observation at night at tick 70](../concepts/micro-world-observation-night.png) |
